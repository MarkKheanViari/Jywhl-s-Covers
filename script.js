...
// Detect user interaction to unlock audio context
...
window.addEventListener("scroll", () => {
  ...
  const fadeElements = document.querySelectorAll(
    ".cover-card, .featured-content, .about-content"
  );
...
// Show playback blocked message
function showPlaybackBlockedMessage() {
  const existingMessage = document.querySelector(".playback-blocked-message");
  if (existingMessage) return;

  const message = document.createElement("div");
  message.className = "playback-blocked-message";
  ...
  `;
  message.textContent = "Please interact with the page to enable audio playback.";
  ...
  ["touchstart", "click"].forEach((eventType) => {
    ...
    prevBtn.addEventListener(
      ...
      (e) => {
        ...
        currentTrackIndex =
          (currentTrackIndex - 1 + coversData.length) % coversData.length;
        loadTrack(currentTrackIndex);
        updateCoverDetails(currentTrackIndex);
      },
      { passive: false }
...
setTimeout(() => {
  document
    .querySelectorAll(".cover-card, .featured-content, .about-content")
    .forEach((el) => {
...
const coverImage = document
  .getElementById("cover-image-large")
  ?.querySelector("img");
...
if (coverArtistSubtitle)
  coverArtistSubtitle.textContent = `By ${cover.artist}`;
...
if (coverOriginal)
  coverOriginal.textContent = `${cover.originalSong} by ${cover.originalArtist}`;
...
if (moreCoversContainer) {
  const relatedCovers = coversData.filter(
    (c) => c.artist === cover.artist && c.id !== cover.id
  );
  ...
  if (relatedCovers.length === 0) {
    ...
    const playCoverButtons =
      moreCoversContainer.querySelectorAll(".play-cover-btn");
      ...
      ["touchstart", "click"].forEach((eventType) => {
        button.addEventListener(
          ...
          (e) => {
            ...
            const relatedCoverIndex = coversData.findIndex(
              (c) => c.audioUrl === button.dataset.audio
            );
            const audio = document.getElementById("cover-audio");
            const playBtn = document.getElementById("play-btn");
            const playIcon = document.getElementById("play-icon");
  ...
  ["touchstart", "click"].forEach((eventType) => {
    button.addEventListener(
      ...
      (e) => {
        ...
        const coverIndex = coversData.findIndex(
          (c) => c.audioUrl === button.dataset.audio
        );
...
function applyFilters() {
  ...
  if (selectedArtist !== "all") {
    filteredCovers = filteredCovers.filter(
      (cover) => cover.artist === selectedArtist
    );
  }

  if (selectedGenre !== "all") {
    filteredCovers = filteredCovers.filter(
      (cover) => cover.genre === selectedGenre
    );
  }
